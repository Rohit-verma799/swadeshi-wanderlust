...
const getAmenityDescription = (name: string) => {
  const descriptions: { [key: string]: string } = {
    'Free WiFi': 'High-speed internet access throughout your stay',
    'AC Transportation': 'Comfortable air-conditioned vehicles for all transfers',
    'Traditional Meals': 'Authentic local cuisine included in package',
    'Welcome Drinks': 'Refreshing beverages upon arrival',
    'Travel Insurance': 'Comprehensive coverage for peace of mind',
    '24/7 Support': 'Round-the-clock assistance during your trip',
    'Local Transport': 'Easy access to local transportation',
    'Beach Transfers': 'Convenient shuttles to nearby beaches',
    'Seafood Meals': 'Fresh local seafood dining experiences',
    'Beach Cafes': 'Access to beachside dining spots',
    'Water Sports': 'Equipment and guidance for water activities',
    'Mountain Transport': 'Safe transport in mountainous terrain',
    'Satvik Meals': 'Pure vegetarian meal options available',
    'Herbal Teas': 'Selection of traditional herbal beverages',
    'Safety Gear': 'Essential equipment for activities',
    'Emergency Support': 'Immediate assistance when needed',
    'Limited WiFi': 'Basic internet connectivity in common areas',
    'Kashmiri Cuisine': 'Traditional Kashmiri food experiences',
    'Kahwa Tea': 'Traditional Kashmiri green tea service',
    'Winter Gear': 'Appropriate clothing for cold weather',
    'Heritage Cafes': 'Dining in historic settings',
    'Monument Fees': 'Entry tickets to monuments included',
    'Mughlai Meals': 'Royal Mughlai cuisine experiences',
    'Spice Tours': 'Guided tours of local spice plantations',
  };
  return descriptions[name] || 'Service included in your package';
}
    ...
    <div className="mb-8">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {destinationsData.map((dest) => (
          <Link 
            key={`quick-${dest.id}`}
            to={`/detailed-itinerary/${dest.id}/mid/5`}
            className="flex-none"
          >
...
const NavigationApp = () => {
  ...
  // Navigation handlers
  const goToDetails = (destinationId: string) => {
  ...
  if (selectedDestination) {
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        {/* Header */}
        <div className="bg-white border-b border-gray-200 text-gray-900 py-3 sticky top-0 z-50 shadow-sm backdrop-blur-lg bg-white/90">
          <div className="flex items-center justify-between max-w-7xl mx-auto px-4">
            <div className="flex items-center gap-6">
              <button onClick={goToLanding} className="text-[#ff6600] hover:text-[#ff8533] transition-colors p-2 rounded-full hover:bg-gray-100">
            ...
            <div className="flex items-center gap-4">
              <Button variant="outline" className="border-[#ff6600] text-[#ff6600] hover:bg-[#ff6600] hover:text-white gap-2">
              ...
              <Card className="shadow-sm border-0 bg-white rounded-xl">
                <CardContent className="p-8">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <Badge variant="outline" className="text-[#ff6600] border-[#ff6600]">{data.duration}</Badge>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {data.highlights.map((highlight, index) => (
                      <div 
                        key={index} 
                        className="relative bg-[#ff6600]/5 rounded-xl p-6 hover:bg-[#ff6600]/10 transition-all group"
                      >
                        <div className="flex items-start gap-4">
                          <div className="w-10 h-10 rounded-lg bg-[#ff6600] text-white flex items-center justify-center shrink-0">
                            <span className="font-bold">D{Math.floor(index * (5 / data.highlights.length) + 1)}</span>
                          </div>
                          <div>
                            <p className="text-gray-900 leading-relaxed">{highlight}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <CardContent className="p-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {data.amenities.map((amenity, index) => (
                      <div 
                        key={index} 
                        className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 bg-white hover:border-[#ff6600]/30 hover:shadow-md transition-all group"
                      >
                        <div className="w-12 h-12 rounded-xl bg-[#ff6600]/10 flex items-center justify-center group-hover:bg-[#ff6600]/20 transition-colors">
                          <amenity.icon className="w-6 h-6 text-[#ff6600]" />
                        </div>
                        <div>
                  ...
                  <div className="flex items-center justify-between mb-8">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} className={`w-5 h-5 ${i < Math.floor(data.rating) ? 'text-[#ff6600] fill-[#ff6600]' : 'text-gray-300 fill-gray-300'}`} />
                          ))}
                        </div>
                        <span className="text-[#ff6600] font-semibold">{data.rating}</span>
                        <span className="text-gray-500">({data.reviewCount.toLocaleString()} reviews)</span>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    {reviews.map((review, index) => (
                      <div key={index} className="bg-gray-50 rounded-xl p-6 hover:bg-gray-100 transition-colors">
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 bg-[#ff6600] rounded-full flex items-center justify-center text-white font-bold text-lg shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-[#ff6600] font-medium">{review.rating}.0</span>
                                <div className="flex">
              ...
              <Card className="border-0 rounded-2xl sticky top-28 shadow-xl bg-white overflow-hidden">
                <CardContent className="p-0">
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <div className="border-2 border-[#ff6600]/10 rounded-xl p-4 bg-[#ff6600]/5">
                    ...
                    <div className="border-2 border-[#ff6600]/10 rounded-xl p-4 bg-[#ff6600]/5">
                      ...
                      <div className="space-y-3 mt-3">
                        <div className="flex items-center justify-between">
                          <span className="text-gray-700">Adults (13+)</span>
                          <div className="flex items-center gap-3">
                            <button 
                              onClick={() => setTravelers(prev => ({...prev, adults: Math.max(1, prev.adults - 1)}))}
                              className="w-8 h-8 rounded-lg bg-white border border-[#ff6600] flex items-center justify-center text-[#ff6600] hover:bg-[#ff6600] hover:text-white transition-colors"
                            >-</button>
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-gray-700">Children (2-12)</span>
                          <div className="flex items-center gap-3">
                            <button 
                    ...
                    <div className="border-t-2 border-dashed border-gray-200 pt-4">
                      <div className="space-y-2">
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-gray-600">Base fare ({travelers.adults + travelers.children} travelers)</span>
                          <span className="font-medium">₹{(parseInt(data.pricing.price.replace(/[^0-9]/g, '')) * (travelers.adults + travelers.children)).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-gray-600">Taxes & fees</span>
                          <span className="font-medium">₹{(1200 * (travelers.adults + travelers.children)).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center text-lg font-bold text-[#ff6600] pt-2">
                          <span>Total Amount</span>
                          <span>₹{((parseInt(data.pricing.price.replace(/[^0-9]/g, '')) + 1200) * (travelers.adults + travelers.children)).toLocaleString()}</span>
                        </div>
                      </div>
                    </div>

                    <Button className="w-full bg-[#ff6600] hover:bg-[#ff6600]/90 text-white font-semibold py-6 rounded-xl transition-all transform hover:scale-105 shadow-lg text-lg">
                      Book Now
                    </Button>

                    <Button variant="outline" className="w-full border border-[#ff6600] text-[#ff6600] hover:bg-[#ff6600]/5 font-semibold py-6 rounded-xl transition-all flex items-center justify-center gap-2">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white max-w-4xl mx-auto px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Link to="/itinerary" onClick={(e) => { e.preventDefault(); navigate('/itinerary'); window.scrollTo(0, 0); }}>
            ...
            <Link to="/businesses" onClick={(e) => { e.preventDefault(); navigate('/businesses'); window.scrollTo(0, 0); }}>
              <Button size="lg" className="text-lg px-10 py-4 bg-white/10 text-white border-2 border-white hover:bg-white hover:text-purple-900 transform hover:scale-105 transition-all duration-300 backdrop-blur-sm font-semibold">
                Explore Businesses
                <ArrowRight className="ml-2 w-5 h-5" />
              </Button>
            </Link>
